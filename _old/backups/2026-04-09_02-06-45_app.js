...
function renderEffects() {
  ...
  // Draw base image (original or grayscale)
  if (S.fx.colorMode) {
    ctx.drawImage(S.img, ox, oy, dw, dh);
  } else {
  ...
  // Draw visible baked layers bottom to top
  for (let i = 0; i < S.bakeStack.length; i++) {
    const layer = S.bakeStack[i];
    if (layer.visible) {
      ctx.drawImage(layer.overlayCanvas, ox, oy, dw, dh);
    }
  }

  ctx.restore();

  // Draw real-time overlays (checker, markers, noise) – these are not baked yet
  const checkerCellSize = DEPTHS[S.fx.checkerDepth];
  const brushCellSize   = DEPTHS[S.fx.brushDepth];
  // Checker (if enabled and mask present)
  if (S.fx.checker && S.mask) {
    const cszS = checkerCellSize * S.vz;
  ...
  if (S.fx.edgeMode !== 'none' || S.fx.rail2 || S.fx.rail3) {
    drawMarkers(ctx, iw, ih, ox, oy, fg, bg, S.fx.seed, S.vz);
  }

  // Noise
  if (S.fx.noise) {
    drawNoise(ctx, iw, ih, ox, oy, fg, bg, S.fx.seed, S.vz);
  }
...
// getRailCells -- module-scope, reused by drawMarkers and drawNoise
// Returns all cell [col, row] pairs at exact Chebyshev distance from grid border
function getRailCells(cols, rows, distance) {
  const cells = [];
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      const minDist = Math.min(r, rows - 1 - r, c, cols - 1 - c);
      if (minDist === distance) cells.push([c, r]);
    }
  }
  return cells;
}
...
// Rail marker drawing
function drawMarkers(ctx, iw, ih, ox, oy, fg, bg, seed, vz) {
  const cellSize = DEPTHS[S.fx.structureDepth];
  const cols = Math.floor(iw / cellSize);
  const rows = Math.floor(ih / cellSize);
  if (cols < 5 || rows < 5) return; // need at least 5 cells to have rails

  const railOffsets = { 1: 101, 2: 202, 3: 303 };
  const step = 3; // fixed step for sparse placement

  // Border mode: continuous solid border ring (edgeMode === 'border')
  if (S.fx.edgeMode === 'border') {
    ctx.fillStyle = rgb(fg);
    const borderCells = getRailCells(cols, rows, 0);
    for (let [c, r] of borderCells) {
      ctx.fillRect(ox + c * cellSize * vz, oy + r * cellSize * vz, cellSize * vz, cellSize * vz);
    }
  }

  // Rail scatter: Rail 1 only when edgeMode === 'rail1'; Rail 2/3 independent
  const activeRails = [];
  if (S.fx.edgeMode === 'rail1') activeRails.push(1);
  if (S.fx.rail2) activeRails.push(2);
  if (S.fx.rail3) activeRails.push(3);
  for (let rail of activeRails) {
      const railSeed = seed + railOffsets[rail];
      let rng = railSeed;
      const rand = () => { rng = (rng * 1664525 + 1013904223) & 0xFFFFFFFF; return (rng >>> 0) / 0x100000000; };

      const cells = getRailCells(cols, rows, rail - 1);
      // Shuffle deterministically
      for (let i = cells.length - 1; i > 0; i--) {
        const j = Math.floor(rand() * (i + 1));
        [cells[i], cells[j]] = [cells[j], cells[i]];
      }
      // Place markers on every step-th cell
...
// HTW noise drawing -- v0.6.5g: edgeMode replaces rail1/rail1Border; bidirectional checker blocking
// Noise never appears over checker cells, rail cells, border cells, or baked visible layers
function drawNoise(ctx, iw, ih, ox, oy, fg, bg, seed, vz) {
  const motherCS = DEPTHS[S.fx.structureDepth];
  const cols = Math.max(1, Math.floor(iw / motherCS));
  const rows = Math.max(1, Math.floor(ih / motherCS));
  if (cols === 0 || rows === 0) return;

  // Build blocked cell set -- all cells already occupied by other effects
  const blocked = new Set();
  const key = (c, r) => r * cols + c;

  // Block checker-masked cells -- bidirectional resolution mapping (both from DEPTHS powers of 2)
  if (S.fx.checker && S.mask) {
    const checkerCS = DEPTHS[S.fx.checkerDepth];
    if (motherCS >= checkerCS) {
      // Case A: one noise cell may overlap multiple checker cells
      const ratio = motherCS / checkerCS;
      for (let r = 0; r < rows; r++) for (let c = 0; c < cols; c++) {
        let hit = false;
        outer: for (let dr = 0; dr < ratio; dr++) for (let dc = 0; dc < ratio; dc++) {
          const mc = c * ratio + dc, mr = r * ratio + dr;
          if (mc < S.mw && mr < S.mh && S.mask[mr * S.mw + mc]) { hit = true; break outer; }
        }
        if (hit) blocked.add(key(c, r));
      }
    } else {
      // Case B: one checker cell overlaps multiple noise cells
      const ratio = checkerCS / motherCS;
      for (let mr = 0; mr < S.mh; mr++) for (let mc = 0; mc < S.mw; mc++) {
        if (!S.mask[mr * S.mw + mc]) continue;
        for (let dr = 0; dr < ratio; dr++) for (let dc = 0; dc < ratio; dc++) {
          const nc = mc * ratio + dc, nr = mr * ratio + dr;
          if (nc < cols && nr < rows) blocked.add(key(nc, nr));
        }
      }
    }
  }

  // Block structure overlays: edgeMode (border/rail1) and rails 2/3
  if (S.fx.edgeMode !== 'none' || S.fx.rail2 || S.fx.rail3) {
    const railOffsets = { 1: 101, 2: 202, 3: 303 };
    const step = 3;

    if (S.fx.edgeMode === 'border') {
      // Border mode: block entire outermost ring
      const bCells = getRailCells(cols, rows, 0);
      for (let i = 0; i < bCells.length; i++) {
        blocked.add(key(bCells[i][0], bCells[i][1]));
      }
    } else if (S.fx.edgeMode === 'rail1') {
      // Rail 1 scatter: block placed marker cells only
      const railSeed1 = seed + railOffsets[1];
      let rng2 = railSeed1;
      const rand2 = () => { rng2 = (rng2 * 1664525 + 1013904223) & 0xFFFFFFFF; return (rng2 >>> 0) / 0x100000000; };
      const rCells1 = getRailCells(cols, rows, 0);
      for (let i = rCells1.length - 1; i > 0; i--) {
        const j = Math.floor(rand2() * (i + 1));
        const tmp = rCells1[i]; rCells1[i] = rCells1[j]; rCells1[j] = tmp;
      }
      for (let i = 0; i < rCells1.length; i += step) {
        blocked.add(key(rCells1[i][0], rCells1[i][1]));
      }
    }

    // Rails 2 and 3: always scatter, always independent of edgeMode
    for (const rail of [2, 3]) {
      if (!S.fx['rail' + rail]) continue;
      const railSeed = seed + railOffsets[rail];
      let rng2 = railSeed;
      const rand2 = () => { rng2 = (rng2 * 1664525 + 1013904223) & 0xFFFFFFFF; return (rng2 >>> 0) / 0x100000000; };
      const rCells = getRailCells(cols, rows, rail - 1);
      for (let i = rCells.length - 1; i > 0; i--) {
        const j = Math.floor(rand2() * (i + 1));
        const tmp = rCells[i]; rCells[i] = rCells[j]; rCells[j] = tmp;
      }
      for (let i = 0; i < rCells.length; i += step) {
        blocked.add(key(rCells[i][0], rCells[i][1]));
      }
    }
  }

  // Block cells covered by visible baked overlay layers
  // Composite all visible layers into one canvas then scan per-cell for any non-transparent pixel
  // Single getImageData call per drawNoise invocation regardless of layer count
  if (S.bakeStack.length) {
    const compCanvas = new OffscreenCanvas(iw, ih);
    const compCtx = compCanvas.getContext('2d');
    for (let li = 0; li < S.bakeStack.length; li++) {
      if (S.bakeStack[li].visible) compCtx.drawImage(S.bakeStack[li].overlayCanvas, 0, 0);
    }
    const compData = compCtx.getImageData(0, 0, iw, ih).data;
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        if (blocked.has(key(c, r))) continue;
        let hit = false;
        for (let py = r * cellSize; py < (r + 1) * cellSize && py < ih && !hit; py++) {
          for (let px = c * cellSize; px < (c + 1) * cellSize && px < iw; px++) {
            if (compData[(py * iw + px) * 4 + 3] > 0) { hit = true; break; }
          }
        }
        if (hit) blocked.add(key(c, r));
      }
    ...
    for (let c = 0; c < cols; c++) {
      ...
      if (rand() < prob) {
        ...
        for (let p = 0; p < clusterSize; p++) {
          ...
          ctx.fillRect(x, y, Math.max(1, Math.floor(vz)), Math.max(1, Math.floor(vz)));
        }
      }
    }
  }
...
// Bake: store current overlays as a new independent layer
function bake() {
  if (!S.img) return;
  // Require a painted mask zone -- baking rails/noise alone produces no committed authoring
  const hasMask = S.mask && S.mask.some(v => v);
  if (!hasMask) { msg('Paint a zone first'); return; }
  const iw = S.img.naturalWidth, ih = S.img.naturalHeight;
  const pal = P2[S.fx.palIdx], fg = pal.c[0], bg = pal.c[1];

  // Create a transparent overlay canvas at native resolution
  const overlay = createOffscreenCanvas(iw, ih);
  const octx = overlay.getContext('2d');
  octx.clearRect(0, 0, iw, ih);
  octx.imageSmoothingEnabled = false;

  const cellSize = DEPTHS[S.fx.checkerDepth];

  // Draw checker if enabled and mask present
  if (S.fx.checker && S.mask) {
    for (let my = 0; my < S.mh; my++) for (let mx = 0; mx < S.mw; mx++) {
      if (!S.mask[my * S.mw + mx]) continue;
      octx.fillStyle = rgb(((mx + my) & 1) === 0 ? fg : bg);
      octx.fillRect(mx * cellSize, my * cellSize, cellSize, cellSize);
    }
  }

  // Structure overlays (border/rails/noise) remain live -- not baked into committed layers.
  // Bake commits checker content only.
  S.bakeStack.push({ overlayCanvas: overlay, visible: true });
  S.mask = null; // clear mask after baking
  renderEffects();
  msg(`Baked layer ${S.bakeStack.length}`);
}
...
function clearBakes() {
  ...
  S.mask = null;
  renderEffects();
  msg('All bakes cleared');
}

// Mask handling (unchanged except brushSize interpretation)
function initMask() {
  if (!S.img) return;
  const cellSize = DEPTHS[S.fx.checkerDepth];
  const mw = Math.max(1, Math.floor(S.img.naturalWidth / cellSize)), mh = Math.max(1, Math.floor(S.img.naturalHeight / cellSize));
...
function screenToMask(mx, my) {
  ...
  const ix = (lx - ox) / S.vz, iy = (ly - oy) / S.vz;
  const cellSize = DEPTHS[S.fx.checkerDepth];
...
function paintMask(mx, my, val, pixelRadius) {
  if (!S.mask) return;
  const p = screenToMask(mx, my); if (!p) return;
  const [cc, cr] = p;
  const cellSize = DEPTHS[S.fx.checkerDepth];
  const cellRadius = Math.max(1, Math.floor(pixelRadius / cellSize));
...
// Helper for icon grid (unchanged)
...
function exportPNG() {
  ...
  if (S.mode === 'effects') {
    ...
    // Draw base image
    if (S.fx.colorMode) {
      ex.drawImage(S.img, 0, 0);
    } else {
      ex.filter = 'grayscale(1)';
      ex.drawImage(S.img, 0, 0);
      ex.filter = 'none';
    }
    ...
    for (let i = 0; i < S.bakeStack.length; i++) {
      const layer = S.bakeStack[i];
      if (layer.visible) {
        ex.drawImage(layer.overlayCanvas, 0, 0);
      }
    }

    // Draw current real-time overlays (if not baked)
    const cellSize = DEPTHS[S.fx.checkerDepth];
    const pal = P2[S.fx.palIdx], fg = pal.c[0], bg = pal.c[1];
    ...
    if (S.fx.checker && S.mask) {
      for (let my = 0; my < S.mh; my++) for (let mx = 0; mx < S.mw; mx++) {
        if (!S.mask[my * S.mw + mx]) continue;
        ex.fillStyle = rgb(((mx + my) & 1) === 0 ? fg : bg);
        ex.fillRect(mx * cellSize, my * cellSize, cellSize, cellSize);
      }
    }
...
const tog = (id, l, on, k = '') => `<div class="tog${on ? ' on' : ''}" data-t="${id}"><span class="togl">${l}</span>${k ? `<span class="togk">[${k}]</span>` : ''}<div class="togp${on ? ' on' : ''}">${on ? 'ON' : 'OFF'}</div></div>`;
...
function sbEffects() {
  const fx = S.fx;
  const hasMask = S.mask && S.mask.some(v => v);
  const hasLayers = S.bakeStack.length > 0;

  let layersHtml = '';
  if (hasLayers) {
    layersHtml = '<div class="sbs"><span class="sbl">LAYERS</span>';
    for (let i = 0; i < S.bakeStack.length; i++) {
      const layer = S.bakeStack[i];
    ...
    layersHtml += '</div><div class="sbs"><div class="sbdiv"></div></div>';
  }

  return sec('PALETTE [P]', p2g(fx.palIdx, P2, 'data-pfx'))
    + sec('EDGE MODE',
      brow([
        { id: 'edgeMode-none',   l: 'None',   on: fx.edgeMode === 'none' },
        { id: 'edgeMode-border', l: 'Border', on: fx.edgeMode === 'border' },
        { id: 'edgeMode-rail1',  l: 'Rail 1', on: fx.edgeMode === 'rail1' }
      ])
      + sec('STRUCTURE SIZE', deptBtns('sdepth', fx.structureDepth))
      + tog('rail2', 'Rail 2', fx.rail2)
      + tog('rail3', 'Rail 3', fx.rail3)
      + sld('seed', 'Seed', fx.seed, 0, 9999))
    + sec('CHECKER',
    ...
    + sec('ACTIONS',
      ...
      + `<button class="abtn danger${hasLayers ? '' : ' disabled-cta'}" id="eclr"${hasLayers ? '' : ' disabled'}>Delete all layers</button>`)
    + layersHtml
    + sec('BASE IMAGE', tog('ecolor', 'Color', fx.colorMode, 'C'));
}
...
function wireUI() {
  ...
  ns.addEventListener('click', e => {
    const t = e.target.closest('[id],[data-t],[data-p2],[data-pfx],[data-p3],[data-p4],[data-mono],[data-layer-visible],[data-layer-delete]');
    if (!t) return;
    ...
    if (t.dataset.layerVisible !== undefined) {
      const idx = +t.dataset.layerVisible;
      if (idx >= 0 && idx < S.bakeStack.length) {
        S.bakeStack[idx].visible = !S.bakeStack[idx].visible;
        renderEffects();
        updateSidebar();
      }
      return;
    }
    if (t.dataset.layerDelete !== undefined) {
      const idx = +t.dataset.layerDelete;
      if (idx >= 0 && idx < S.bakeStack.length) {
        S.bakeStack.splice(idx, 1);
        renderEffects();
        updateSidebar();
        msg(`Layer ${idx + 1} deleted`);
      }
      return;
    }
    ...
    if (id.startsWith('sdepth-')) { S.fx.structureDepth = +id.split('-')[1]; dProc(); return; }
    if (id.startsWith('cdepth-')) { S.fx.checkerDepth = +id.split('-')[1]; S.mask = null; dProc(); return; }
    if (id.startsWith('bdepth-')) { S.fx.brushDepth = +id.split('-')[1]; dProc(); return; }
    if (id.startsWith('edgeMode-')) { S.fx.edgeMode = id.replace('edgeMode-', ''); dProc(); return; }
...
function handleToggle(id) {
  switch (id) {
    ...
    case 'rail2': S.fx.rail2 = !S.fx.rail2; break;
    case 'rail3': S.fx.rail3 = !S.fx.rail3; break;
    case 'noise': S.fx.noise = !S.fx.noise; break;
  }
  dProc();
}

// Undo/redo for grid (icon/bitmap) – unchanged
const _undo = [], _redo = [];
...
document.addEventListener('keydown', e => {
  ...
  switch (e.key) {
    ...
    case 'j': case 'J': if (S.mode === 'effects') { S.fx.edgeMode = S.fx.edgeMode === 'rail1' ? 'none' : 'rail1'; dProc(); } break;
  }
});
document.addEventListener('keyup', e => { if (e.key === ' ') S.spaceDown = false; });

let _rectStart = null;
let _curCell = null; // current [c,r] under cursor in effects mode
cw.addEventListener('mousedown', e => {
  ...
  if (S.mode === 'effects') {
    if (S.tool === 'brush' || S.tool === 'eraser') {
      ...
      paintMask(e.clientX, e.clientY, S.paintVal, DEPTHS[S.fx.brushDepth]);
      renderEffects();
    }
...
cw.addEventListener('mousemove', e => {
  ...
  if (S.mode === 'effects') {
    if (S.painting && S.tool !== 'move') {
      paintMask(e.clientX, e.clientY, S.paintVal, DEPTHS[S.fx.brushDepth]);
      renderEffects();
    } else if (S.tool !== 'move') {
      // Cursor tracking -- rerender only when cell changes or enters/exits grid
      const newCell = screenToMask(e.clientX, e.clientY);
      const changed = !newCell !== !_curCell || (newCell && _curCell &&
        (newCell[0] !== _curCell[0] || newCell[1] !== _curCell[1]));
      if (changed) { _curCell = newCell; renderEffects(); }
    }
  }
});
cw.addEventListener('mouseleave', () => {
  if (_curCell !== null) { _curCell = null; if (S.mode === 'effects') renderEffects(); }
});
